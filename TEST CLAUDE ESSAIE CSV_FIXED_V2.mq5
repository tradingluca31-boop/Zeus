...
double LotsFromRisk_XAUUSD_Fixed(int dir, double entry, double sl)
{
   ...
   Print("=====================================================================");
   Print("              CALCUL LOT SIZE ULTRA-SIMPLIFIE XAUUSD             ");
   Print("=====================================================================");
   ...
   if(UseLossStreakReduction) {
      ...
      if(gLossStreak >= LossStreakTrigger) {
         ...
         Print("ALERTE - REDUCTION RISQUE: ", gLossStreak, " pertes -> ", riskAmount, "$");
      }
   }
...
{
   Print("=== OnDeinit appele - Raison: ", reason, " ===");

   if(MQLInfoInteger(MQL_TESTER)) {
      Print("OnDeinit: Mode testeur detecte - Lancement export de sauvegarde");
   ...
   Print("OnDeinit: Handles liberes");
}
...
void ExportTradeHistoryCSV()
{
   ...
   {
      Print("Echec FILE_COMMON, essai sans FILE_COMMON");
      file_handle = FileOpen(file_name, FILE_WRITE | FILE_ANSI, 0, CP_UTF8);
   }
   ...
   {
      Print("Historique selectionne avec succes");
      ...
      Print("Nombre de trades exportes: ", exported_count);
   }
   else
   {
      Print("ERREUR: Impossible de selectionner l'historique. Erreur: ", GetLastError());
   }
